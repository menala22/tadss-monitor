...
def fetch_position_with_signals_simple(position: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Get quick signal status (simplified)
        bullish_count = 0
        bearish_count = 0
        signal_summary = "NEUTRAL"
        
        # Simple signal based on price vs entry
        if pnl_pct > 2:
            bullish_count = 3
...
def fetch_position_with_signals(position: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        bullish_count = sum(
            1 for s in [signal_states.get("MA10"), signal_states.get("MA20"), signal_states.get("MA50"), signal_states.get("MACD"), signal_states.get("RSI")]
            if s in ["BULLISH", "OVERBOUGHT"]
        )
        bearish_count = sum(
            1 for s in [signal_states.get("MA10"), signal_states.get("MA20"), signal_states.get("MA50"), signal_states.get("MACD"), signal_states.get("RSI")]
            if s in ["BEARISH", "OVERSOLD"]
        )
        ...
        if is_long and bearish_count >= 4:
            health_status = "CRITICAL"
        elif not is_long and bullish_count >= 4:
            health_status = "CRITICAL"
        elif is_long and bearish_count >= 2:
            health_status = "WARNING"
        elif not is_long and bullish_count >= 2:
            health_status = "WARNING"
        else:
            health_status = "HEALTHY"
...
def render_position_detail(position_data: Dict[str, Any]) -> None:
    ...
    # Header
    st.title(f"📍 {pair} - {direction}")
    
    # Direction emoji
    direction_emoji = "🟢" if direction == "LONG" else "🔴"
    st.caption(f"{direction_emoji} {direction} Position")
    ...
    # Crypto: everything else (e.g., BTCUSD, ETH, BTC-USD)
    is_likely_stock = pair_clean.isalpha() and len(pair_clean) <= 5 and pair_clean not in ["BTC", "ETH", "SOL", "DOGE", "XRP", "ADA", "DOT"]
    primary_source = "yfinance" if is_likely_stock else "ccxt"
...
def render_settings_page() -> None:
    ...
    st.info(f"Current interval: {settings.monitor_interval / 3600:.0f} hours ({settings.monitor_interval} seconds)")
    st.markdown(
        """
        To change the monitoring interval, edit your `.env` file:
