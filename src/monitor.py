...
class PositionMonitor:
    ...
    def _fetch_position_data(
        ...
        NEW (Option 2): Uses pre-calculated signals from technical_signals table
        when available, falling back to on-the-fly calculation if not.
        ...
        """
        try:
            # Try 1: Get pre-calculated signals from technical_signals table (FAST PATH)
            signal_data = self._get_cached_signals(position.pair, position.timeframe)
            ...
            if signal_data:
                ...
                logger.debug(
                    f"Using cached signals for {position.pair} {position.timeframe} "
                    f"({data_points} candles)"
                )
            ...
            # Try 2: Get data from ohlcv_universal (read-only, no API calls)
            ...
            # Calculate technical signals on-the-fly (fallback)
    ...
    def _get_cached_signals(
        ...
        """
        Get pre-calculated signals from technical_signals table.

        This is the FAST PATH for position scanning - signals are already
        calculated and stored, so we just need to query and convert them.

        Args:
            pair: Trading pair symbol (e.g., 'ETHUSD').
            timeframe: Timeframe (e.g., 'h1').
            limit: Number of candles to retrieve (default: 100).

        Returns:
            Dictionary with signal object and current price, or None if not found.
        """
        from src.models.ohlcv_universal_model import normalize_symbol
        from src.models.technical_signal_model import TechnicalSignal
        from src.database import get_db_context

        try:
            with get_db_context() as db:
                # Normalize symbol to match technical_signals format
                normalized_pair = normalize_symbol(pair)

                # Get latest pre-calculated signals
                signals = db.query(TechnicalSignal).filter(
                    TechnicalSignal.symbol == normalized_pair,
                ...
                if not signals:
                    logger.debug(
                        f"No cached signals for {pair} {timeframe} "
                        f"(normalized: {normalized_pair})"
                    )
                    return None

                # Get latest signal (most recent candle)
                latest_signal = signals[0]

                # Convert to TechnicalSignal format compatible with TechnicalAnalyzer
                signal_states = latest_signal.to_signal_states()

                # Create a minimal TechnicalSignal-like object
                from src.services.technical_analyzer import TechnicalSignal as TASignal
                from src.services.technical_analyzer import SignalState

                # Determine overall status from cached signals
                overall_status = self._determine_overall_status({
                    'MA10': latest_signal.signal_ma10,
                    'MA20': latest_signal.signal_ma20,
                    'MA50': latest_signal.signal_ma50,
                    'MACD': latest_signal.signal_macd,
                    'RSI': latest_signal.signal_rsi,
                    'OTT': latest_signal.signal_ott,
                })

                # Get current price from ohlcv_universal
                ohlcv = db.query(OHLCVUniversal).filter(
                    OHLCVUniversal.symbol == normalized_pair,
                ...
                signal = TASignal(
                    pair=pair,
                    position_type=PositionType.LONG,  # Placeholder - not used for status calculation
                    timeframe=timeframe,
                    current_price=current_price,
                    signal_states=signal_states,
                    overall_signal=SignalState(overall_status),
                    bullish_count=0,  # Not needed for position monitoring
                    bearish_count=0,
                    neutral_count=0,
                )
            ...
            logger.error(f"Failed to get cached signals for {pair} {timeframe}: {e}")
            return None
    ...
    def _get_data_from_universal(
        ...
        from src.models.ohlcv_universal_model import OHLCVUniversal, normalize_symbol
        from src.database import get_db_context

        try:
            with get_db_context() as db:
                # Normalize symbol to match ohlcv_universal format (e.g., XAUUSD → XAU/USD)
                normalized_pair = normalize_symbol(pair)
