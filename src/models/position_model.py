...
class Position(Base):
    ...
    Attributes:
        ...
        last_important_indicators: Last recorded MA10 and OTT status (nullable).
    ...
    last_ma10_status = Column(String(20), nullable=True)  # MA10 status (e.g., "BULLISH", "BEARISH")
    last_ott_status = Column(String(20), nullable=True)   # OTT status (e.g., "BULLISH", "BEARISH")
    last_important_indicators = Column(String(50), nullable=True)  # DEPRECATED: Kept for backward compatibility
...
def migrate_add_signal_columns(database_url: str = "sqlite:///./positions.db") -> bool:
    ...
    - last_important_indicators: String column for storing MA10 and OTT status
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if 'last_important_indicators' not in columns:
                conn.execute(text(
                    "ALTER TABLE positions ADD COLUMN last_important_indicators VARCHAR(50)"
                ))
                print("✓ Added last_important_indicators column")
