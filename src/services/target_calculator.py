...
class TargetCalculator:
    ...
    def _select_best_method(
        ...
        # Priority 2: Check for HTF S/R levels
        if htf_bias and htf_bias.key_levels:
            # Check if there's a clear S/R level ahead
            for level in htf_bias.key_levels:
                if direction == "LONG" and level.level_type == LevelType.RESISTANCE:
                    if level.price > df_htf["close"].iloc[-1] * 1.01:
                        logger.debug("Auto-select: S/R Level (clear resistance ahead)")
                        return TargetMethod.SR_LEVEL
                elif direction == "SHORT" and level.level_type == LevelType.SUPPORT:
                    if level.price < df_htf["close"].iloc[-1] * 0.99:
                        logger.debug("Auto-select: S/R Level (clear support ahead)")
                        return TargetMethod.SR_LEVEL

        # Priority 3: Check for strong impulse (Fibonacci)
        if self._is_strong_impulse(df_mtf, direction):
            logger.debug("Auto-select: Fibonacci (strong impulse)")
            return TargetMethod.FIBONACCI

        # Priority 4: Check for range/counter-trend (prior swing)
        if htf_bias and htf_bias.price_structure.value == "RANGE":
            logger.debug("Auto-select: Prior Swing (range market)")
            return TargetMethod.PRIOR_SWING

        # Default: ATR-based
        logger.debug("Auto-select: ATR (default)")
    ...
    def _calculate_sr_target(
        ...
        if not htf_bias or not htf_bias.key_levels:
            ...
            return TargetResult(
                ...
                confidence=0.0,
                description="No S/R levels available",
            )
        ...
        if target_price is None:
            return TargetResult(
                ...
                method=TargetMethod.SR_LEVEL,
                confidence=0.3,
                description="No S/R level in trade direction",
            )
        ...
        return TargetResult(
            ...
            method=TargetMethod.SR_LEVEL,
            confidence=0.7,
            description=f"Next {direction} {level.level_type.value} at {target_price:.4f}",
        )
