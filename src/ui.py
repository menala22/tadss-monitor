                ...
                for line in f:
                    ...
                    if line and not line.startswith('#') and '=' in line:
                        ...
                        if key.strip() == target_key:
                            v = value.strip()
                            if v and not v.startswith('your_') and 'VM_EXTERNAL_IP' not in v:
                                return v
        except Exception:
            pass
    return None
...
API_BASE_URL = (
    (_env_api_url if _env_api_url and "VM_EXTERNAL_IP" not in _env_api_url else None)
    or _load_api_url_from_env()
...
def get_current_api_url() -> str:
    ...
    Priority:
    1. Session state override (from Settings page toggle) — only if it's a real URL
    2. Module-level API_BASE_URL (from env var or .env file)
    ...
    if hasattr(st, "session_state") and "api_base_url_override" in st.session_state:
        ...
        if override and "VM_EXTERNAL_IP" not in override:
            return override
        # Bad override — drop it so we fall through to the real URL
        del st.session_state.api_base_url_override
    # Re-read from env/file in case .env was updated since startup
    fresh = _load_api_url_from_env()
    return fresh or API_BASE_URL
...
def fetch_open_positions_from_api() -> tuple[Optional[List[Dict[str, Any]]], str, str]:
    """
    ...
    Returns:
        (positions, error_type, error_detail)
        positions is None on failure; error_type is one of
        "connection", "timeout", "http", "parse", or "".
    """
    url = f"{get_current_api_url()}/positions/open"
    for attempt in range(2):
        try:
            response = requests.get(url, headers=get_api_headers(), timeout=60)
            response.raise_for_status()
            return response.json(), "", ""
        except requests.exceptions.ConnectionError:
            if attempt == 0:
                continue
            return None, "connection", url
...
def render_sidebar() -> str:
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Select Page",
            ["📋 Open Positions", "➕ Add New Position", "🔍 MTF Scanner", "📈 Market Data", "💼 MTF Opportunities", "⚙️ Settings"],
            index=0,
...
def render_main_page() -> None:
    ...
    api_positions, conn_error_type, conn_error_detail = fetch_open_positions_from_api()
    api_connected = api_positions is not None

    # API Connection Banner
    if not api_connected:
        api_url = get_current_api_url()
        is_vm = "localhost" not in api_url

        if conn_error_type == "timeout":
            st.error(
                f"### ⚠️ API Timeout\n\n"
                f"The VM API did not respond within 60 seconds (tried twice).\n\n"
                f"**URL:** `{api_url}`\n\n"
                f"This usually means the VM is busy (scheduler running at :10/:20/:30). "
                f"Wait 1–2 minutes and retry."
            )
        elif conn_error_type == "http":
            st.error(
                f"### ⚠️ API Error\n\n"
                f"The server returned an error: `{conn_error_detail}`\n\n"
                f"**URL:** `{api_url}`"
            )
        elif conn_error_type == "connection" and is_vm:
            st.error(
                f"### ⚠️ Cannot Reach VM\n\n"
                f"Could not connect to the VM API after 2 attempts.\n\n"
                f"**URL:** `{api_url}`\n\n"
                f"Check that the VM is running and the IP in `.env` is current "
                f"(VM IP changes on restart)."
            )
        else:
            st.error(
                f"### ⚠️ Backend Connection Lost\n\n"
                f"Unable to connect to the API server.\n\n"
                f"**URL:** `{api_url}`\n\n"
                + (
