...
from src.services.mtf_notifier import send_mtf_opportunity_alert, send_new_opportunity_alert
from src.services.mtf_opportunity_scanner import MTFOpportunityScanner, load_pair_data_from_universal
...
# Candle limits imported from scanner (single source of truth).
from src.services.mtf_opportunity_scanner import ROLE_LIMITS as _ROLE_LIMITS  # noqa: E402
...
def _load_pair_data_from_universal(
    ...
    """Delegate to shared implementation in mtf_opportunity_scanner."""
    return load_pair_data_from_universal(pair, config, db)
...
def _format_scan_result(pair: str, scan_result) -> Dict[str, Any]:
    ...
    return {
        ...
        "weighted_score": round(alignment.weighted_score, 2),
        "position_size_pct": round(alignment.position_size_pct, 0),
...
def get_timeframe_configs() -> Dict[str, Any]:
    ...
    return {
        ...
        "description": {
            ...
            "mtf": "MidTF (Middle Timeframe) — setup identification (pullback, divergence)",
...
def trigger_mtf_scan(
    ...
    Always scans both SWING and INTRADAY regardless of watchlist trading_style setting.

    Body: {"pairs": ["BTC/USDT"], "min_alignment": 2, "min_rr_ratio": 2.0}
    """
    ...
    # Always scan both styles — trading_style in body is ignored
    scan_styles = [TradingStyle.SWING, TradingStyle.INTRADAY]
    ...
    for style in scan_styles:
        ...
        for pair in pair_list:
            data = _load_pair_data_from_universal(pair, config, db)
            if data is None:
                logger.warning(f"Skipping {pair} ({style.value}) — no data in ohlcv_universal")
                continue
            try:
                scan_result = scanner.scan_pair_detailed(
                    pair=pair,
                ...
                formatted = _format_scan_result(pair, scan_result)
                formatted["trading_style"] = style.value
                all_results.append(formatted)

                # Save qualifying opportunities to DB
                if opportunity_service.should_save_opportunity(scan_result.alignment):
                    patterns = scanner._detect_patterns(
                        htf_bias=scan_result.alignment.htf_bias,
                        mtf_setup=scan_result.alignment.mtf_setup,
                        ltf_entry=scan_result.alignment.ltf_entry,
                    )
                    divergence_result = scanner.divergence_detector.detect_divergence(data["mtf"])
                    divergence = (
                        divergence_result.latest_type.value
                        if divergence_result.divergences else None
                    )
                ...
                logger.error(f"Scan failed for {pair} ({style.value}): {exc}", exc_info=True)

    # Send alerts for saved opportunities
    for opp in saved_opps:
        if opp.weighted_score >= 0.60:
            try:
                send_new_opportunity_alert(opp)
            except Exception as exc:
                logger.error(f"Failed to send alert for {opp.pair}: {exc}", exc_info=True)

    opportunities = [r for r in all_results if r["passes_filters"]]

    return {
        "timestamp": datetime.utcnow().isoformat(),
        "trading_styles": ["SWING", "INTRADAY"],
        "opportunities": opportunities,
        "summary": {
            "total_scanned": len(all_results),
            "opportunities_found": len(opportunities),
            "saved_to_db": len(saved_opps),
        },
    }
