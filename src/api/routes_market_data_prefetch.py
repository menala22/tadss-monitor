...
def trigger_prefetch(
    ...
    force_backfill: bool = Query(
        default=False,
        description="Delete existing rows for each symbol/timeframe before fetching — forces a full re-fetch up to FETCH_LIMITS",
    ),
    ...
    **Note:** This runs synchronously and may take 30-60 seconds for full refresh.
    ...
    Args:
        ...
        force_backfill: Wipe existing rows first so the full FETCH_LIMITS are used.

    Returns:
        Prefetch result with statistics.

    Example:
        POST /api/v1/market-data/prefetch?force_backfill=true
    """
    global _last_prefetch_result, _last_prefetch_time

    logger.info(f"Manual prefetch triggered (force_backfill={force_backfill})")
    ...
    try:
        ...
        if force_backfill:
            orchestrator.clear_for_backfill(
                symbols=symbol_list,
